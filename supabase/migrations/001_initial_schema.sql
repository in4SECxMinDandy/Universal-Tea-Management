-- ============================================================
-- UniTEA Food Store — Initial Schema Migration
...
CREATE INDEX idx_foods_is_available ON foods(is_available) WHERE deleted_at IS NULL;
CREATE INDEX idx_foods_is_featured ON foods(is_featured) WHERE deleted_at IS NULL;
CREATE INDEX idx_foods_deleted_at ON foods(deleted_at) WHERE deleted_at IS NOT NULL;
...
CREATE TRIGGER foods_soft_delete
  ...
  FOR EACH ROW EXECUTE PROCEDURE set_deleted_at();
...
CREATE TRIGGER visit_session_auto_expires
  ...
  FOR EACH ROW EXECUTE PROCEDURE set_visit_expires_at();
...
CREATE TRIGGER chat_messages_new_message
  ...
  FOR EACH ROW EXECUTE PROCEDURE update_chat_session_last_message();
...
-- Visit sessions
CREATE POLICY "visit_sessions_select" ON visit_sessions FOR SELECT USING (has_role(auth.uid(), 'STORE_ADMIN') OR user_id = auth.uid());
CREATE POLICY "visit_sessions_insert_admin" ON visit_sessions FOR INSERT WITH CHECK (has_role(auth.uid(), 'STORE_ADMIN'));
CREATE POLICY "visit_sessions_update_admin" ON visit_sessions FOR UPDATE USING (has_role(auth.uid(), 'STORE_ADMIN'));

-- Chat sessions
CREATE POLICY "chat_sessions_select" ON chat_sessions FOR SELECT
  USING (has_role(auth.uid(), 'STORE_ADMIN') OR user_id = auth.uid());
CREATE POLICY "chat_sessions_insert_auth" ON chat_sessions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Chat messages
CREATE POLICY "chat_messages_select" ON chat_messages FOR SELECT
  USING (
    has_role(auth.uid(), 'STORE_ADMIN')
    OR session_id IN (SELECT id FROM chat_sessions WHERE user_id = auth.uid())
  );
